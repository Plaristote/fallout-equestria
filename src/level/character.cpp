...
template<typename T>
T DefaultValue(Data data, const T& default_value)
{
  if (data.Nil())
...
{
public:
  static Gfx* Factory(NodePath parent, Data data)
  {
    // Factory
    ...
    particle_sys->set_pool_size    (16384);
    particle_sys->set_birth_rate   (0.25);
    particle_sys->set_litter_size  (256);
    ...
    NodePath np = parent.attach_new_node("Particles");

    np.set_z(np.get_z() - 5);
    np.set_light_off();
    particle_sys->set_render_parent(np);

    return (new Gfx(particle_sys));
  }

private:
  Gfx(PT(ParticleSystem) system) : system(system)
  {
  }
...
void                ObjectCharacter::GoTo(Waypoint* waypoint)
{
  ...
  if (done_once == false)
  {
    ...
    NodePath  np = _level->GetWorld()->floors_node.attach_new_node("EffectBase");

    np.set_pos(GetNodePath().get_pos());
    Gfx* gfx = Gfx::Factory(np, dt);

    delete dt;
    {
      ParticleSystemManager& particle_manager = _level->GetParticleManager();

      particle_manager.attach_particlesystem(gfx->system);
    }

    //Projectile* projectile = new Projectile(_level->GetWorld(), GetNodePath());
